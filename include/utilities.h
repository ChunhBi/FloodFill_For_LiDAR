...
template < typename T > T mean(const T& val1, const T& val2) { return 0.5 * (val1 + val2); }

/* Check if 'val' is within the intervals defined by 'bracket1' and 'bracket2'. Boundaries inclusive. */
template < typename T > bool ix(const T& val, const T& bracket1, const T& bracket2) {
...
template < typename T > bool lix(const T& val, const T& bracket1, const T& bracket2) {
    assert(bracket1 < bracket2);
    return (bracket1 <= val && val < bracket2);
}

/* Check if 'val' is within the intervals defined by 'bracket1' and 'bracket2'. Exclude 'bracket1', include 'bracket2'. */
template < typename T > bool rix(const T& val, const T& bracket1, const T& bracket2) {
    assert(bracket1 < bracket2);
    return (bracket1 < val && val <= bracket2);
}

/* Check if 'val' is within the intervals defined by 'bracket1' and 'bracket2'. Boundaries exclusive. */
template < typename T > bool ex(const T& val, const T& bracket1, const T& bracket2) {
    assert(bracket1 < bracket2);
    return (bracket1 < val && val < bracket2);
}
 ...
 */
template < typename T >
void lineIntersection(const double& x1, const double& y1, const double& x2, const double& y2, const double& x3, const double& y3, const double& x4, const double& y4, double& t, double& u,
...
template < typename T > inline const T& bound(const T& l, const T& val, const T& u) { return max(l, min(u, val)); }

/*
 * Round v to p places after decimal.
 */
inline double round(const double v, const int p) {
 ...
 * Round v to p places after decimal.
 */
inline double trunc(const double v, const int p) {
    const int multiplier = std::pow(10, p);
    return std::trunc(v * multiplier) / multiplier;
}
}  // namespace utils

template < typename T, int R, int C > bool notValid(const Eigen::Matrix< T, R, C >& v) {
